...
The SPA uses an **Inter + JetBrains Mono** light theme with a blue accent (`#2563eb`) and per-type color pills (feedback amber, user green, project blue, reference purple, index teal). Top bar: collapsible sidebar toggle, search box with `Ctrl/Cmd+K` shortcut, type-filter tabs (`all` / `feedback` / `user` / `project` / `reference`), and a `↻` refresh button that re-pulls the project tree (preserves the viewer in edit / confirm-delete mode so unsaved input is never dropped). The viewer pane renders a `<project> / memory / <filename>` breadcrumb, the type pill, title, description, and a Notion-style toolbar (`编辑` · `复制路径` · `在 VSCode 打开` · `删除`). **CRUD inline**: edit mode lets you patch name / description / body (`Ctrl/Cmd+S` to save, `Esc` to cancel); delete shows an in-page confirm card before removing the file. Deletion is restricted to memory entries (`<project>/memory/*.md`) — non-entry files under the projects tree (e.g. `sessions/*.jsonl`) cannot be deleted via the API. Writes use atomic `tempfile + os.replace` so a mid-write crash never leaves a half-written file on disk.

**MEMORY.md index panel** — each project's `MEMORY.md` index file (the human-curated list of entries) shows up as a `📋 索引 (MEMORY.md)` row at the top of its project group, with full view / edit / delete support. Edit mode for the index is a single raw-markdown textarea (no name / description / type fields, since the index has no frontmatter). The index pill renders teal to distinguish it from the four entry types. Internal `[Title](filename.md)` style links inside a rendered MEMORY.md are clickable: clicking jumps directly to the referenced entry's view in the SPA (no browser navigation), so the index acts as a project's table-of-contents. External `http(s)://` links open in a new tab. The index row stays visible regardless of the active type tab — the index is not filtered by type.
